...
class AITriggerEvaluator:
    ...
    - 保険ポーリング: TREND=2分、NORMAL=10分間隔で定期呼び出し
    """
    ...
    def should_call_ai(self, events: list,
        ...
        if events:
            ...
            if elapsed >= 10:
                ...
                logger.debug(
                    f"イベント検出だがクールダウン中 "
                    f"(残り{10 - elapsed:.0f}秒)"
                )

        # === EXTREME相場では呼び出し頻繁に ===
        if volatility_regime == "EXTREME" and elapsed >= 30:
            reason = "EXTREME相場 - 30秒ポーリング"
            return True, reason

        # === ポジション保有中は頻繁にチェック ===
        if has_position and elapsed >= 60:
            reason = "ポジション保有中 - 1分ポーリング"
            return True, reason

        # === 保険ポーリング（定期）===
        if volatility_regime in ("TREND", "HIGH_VOL"):
            interval = self.config.trend_poll_interval  # 2分
        else:
            interval = self.config.normal_poll_interval  # 10分
